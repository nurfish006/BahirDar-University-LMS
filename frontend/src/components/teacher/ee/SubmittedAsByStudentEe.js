...
const SubmittedAsByStudentEe = () => {
  ...
  useEffect(() => {
    const getFilesList = async () => {
      try {
        const { data } = await axios.get(`${API_URL}/getAllFilesia`);
        setErrorMsg('');
        setFilesList(data);
      } catch (error) {
  ...
  const downloadFile = async (id, path, mimetype) => {
    try {
      const result = await axios.get(`${API_URL}/downloadia/${id}`, {
        responseType: 'blob'
      });
            ...
            filesList.map(
              ({ _id, studname, title, description, file_path, file_mimetype }) => (
                <tr key={_id} className='hover:bg-gray-100 transition-all'>
                  ...
                  {/* <td className="py-3 border"><MdDelete size={24} onClick={() => deleteFilecs(_id)} /></td> */}
                </tr>
              )
            )
          ) : (
...
export default SubmittedAsByStudentEe;
