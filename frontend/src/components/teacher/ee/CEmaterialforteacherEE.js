...
const CEmaterialforteacherEE = () => {
  ...
  const deleteFilece = async (id) => {
    try {
      await axios.delete(`${API_URL}/deleteFileee/${id}`);
  ...
  useEffect(() => {
    const getFilesList = async () => {
      try {
        const { data } = await axios.get(`${API_URL}/getAlleeFiles`);
        setErrorMsg('');
        setFilesList(data);
      } catch (error) {
  ...
  const downloadFile = async (id, path, mimetype) => {
    try {
      const result = await axios.get(`${API_URL}/downloadee/${id}`, {
        responseType: 'blob'
      });
    ...
    <div className="mt-3 mx-auto bg-gray-300 max-w-5xl rounded-lg p-6">
      ...
      <div className="flex justify-center mt-10">
        <Button variant="primary" type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
          <Link to="/teacher/ee/addcoursematerial">Add material</Link>
        </Button>
      </div>
    </div></div>
    <div><Example/></div>
...
export default CEmaterialforteacherEE;
