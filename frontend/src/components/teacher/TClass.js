...
function LinksTeacher() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/addnewlinkc', {
        url,
        description
      });
  ...
  async function handleDeleteClick(linkId) {
    try {
      const response = await axios.delete(`http://localhost:8000/api/deletelinkc/${linkId}`);
