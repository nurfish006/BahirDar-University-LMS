...
const AssignmentCe = (props) => {
    ...
    try {
      ...
      if (title.trim() !== '' && description.trim() !== '') {
        if (file) {
          ...
          setErrorMsg('');
          await axios.post(`${API_URL}/uploadceass`, formData, {
            headers: {
  ...
  // deleting materials
  const deleteFileceass = async (id) => {
    try {
      await axios.delete(`${API_URL}/deleteFileceass/${id}`);
      setFilesList(filesList.filter((file) => file._id !== id));
      toast.success('Assignment deleted successfully');
    } catch (error) {
      toast.error('Error while deleting assignment. Try again later');
    }
  };
  ...
  return (
    <div>
      <Navbar/>
      <div className="">
      <div className="container mx-auto py-4 grid grid-cols-2 gap-4 h-screen">
        <div className=''>
...
<Link to="/teacher/ce/uploadassignmentresult">
