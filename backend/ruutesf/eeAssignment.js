...
const EEass = require('../models/ee_assignment');
...
Router.post('/uploadeeass',
  upload.single('file'),
  async (req, res) => {
    try {
      const { title, description } = req.body;
      const { path, mimetype } = req.file;
      const file = new EEass({
        title,
        description,
        file_path: path,
...
Router.get('/getAlleeass', async (req, res) => {
  try {
    const files = await EEass.find({});
...
Router.get('/downloadeeass/:id', async (req, res) => {
  try {
    const file = await EEass.findById(req.params.id);
    res.set({
...
//delete
Router.delete('/deleteFileeeass/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const file = await EEass.findByIdAndDelete(id);
    if (!file) {
