...
const ASSresultce = require('../models/assresultce');
...
Router.post('/uploadresultce',
  upload.single('file'),
  async (req, res) => {
    try {
      const { title, description } = req.body;
      const { path, mimetype } = req.file;
      const file = new ASSresultce({
        title,
        description,
        file_path: path,
...
Router.get('/assresultce', async (req, res) => {
  try {
    const files = await ASSresultce.find({});
...
Router.get('/downloadresult/:id', async (req, res) => {
  try {
    const file = await ASSresultce.findById(req.params.id);
    res.set({
...
Router.delete('/deleteFileca/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const file = await ASSresultce.findByIdAndDelete(id);
    if (!file) {
