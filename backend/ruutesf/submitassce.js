...
const CESASS = require('../models/submitAssce');
...
Router.get('/getAllFilesca', async (req, res) => {
  try {
    const files = await CESASS.find({});
...
Router.get('/downloadca/:id', async (req, res) => {
  try {
    const file = await CESASS.findById(req.params.id);
    res.set({
...
Router.delete('/deleteFileca/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const file = await CESASS.findByIdAndDelete(id);
    if (!file) {
